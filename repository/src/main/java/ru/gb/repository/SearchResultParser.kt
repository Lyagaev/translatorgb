...
import ru.gb.translatorgb.model.data.Meanings
...
fun mapHistoryEntityToSearchResult(list: List<ru.gb.repository.room.HistoryEntity>): List<DataModel> {
...
fun convertDataModelSuccessToEntity(appState: AppState): ru.gb.repository.room.HistoryEntity? {
    return when (appState) {
        is AppState.Success -> {
            ...
            if (searchResult.isNullOrEmpty() || searchResult[0].text.isNullOrEmpty()) {
                ...
                ru.gb.repository.room.HistoryEntity(searchResult[0].text!!, null)
            }
        }
        else -> null
    }
